  ...
  <Tab title="Linux (dev.sh)">
    <Steps>
      ...
      <Step title="Run the development bootstrap">
        ...
        The script validates Docker, creates the `.env` from `.env.example` if it does not yet exist, builds the `Dockerfile.dev` image, starts the support services (MariaDB, Redis, Mailhog, PhpMyAdmin), waits for the database health check to pass, then starts the `app` container and installs PHP and Node dependencies inside it.
  ...
  <Tab title="Windows (dev.ps1)">
    <Steps>
      ...
      <Step title="Run the development bootstrap">
        ...
        The PowerShell script performs the same operations as `dev.sh`: validates Docker Desktop is running, creates `.env` from `.env.example`, builds the development image, starts support services, waits for MariaDB to become healthy, starts the `app` container, and installs Composer and pnpm dependencies inside it.
...
<AccordionGroup>
  ...
  <Accordion title="Run the test suite">
    ...
    ```
    PHPUnit is configured in `phpunit.xml` with an in-memory SQLite database so tests run in isolation without touching the development MariaDB instance.
  </Accordion>
