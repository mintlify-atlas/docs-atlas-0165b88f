---
title: "Email Notifications and Alerts in Leo Counter"
...
| PATCH | `/canal-notificaciones/{id}/{attribute}/toggle` | Toggle `active` flag |
...
| Method | URI | Action |
|--------|-----|--------|
| GET | `/suscriptor-notificaciones` | List all subscribers |
| POST | `/suscriptor-notificaciones` | Add a subscriber |
| DELETE | `/suscriptor-notificaciones/{id}` | Remove a subscriber |
| PATCH | `/suscriptor-notificaciones/{id}/{attribute}/toggle` | Toggle `active` flag |

<Note>
  The edit and update actions are intentionally omitted for subscribers. To change a subscriber's channel, delete the existing subscription and create a new one.
</Note>
