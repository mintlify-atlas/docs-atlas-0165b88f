---
title: "Financial Reports and Analytics in Leo Counter"
sidebarTitle: "Reports"
description: "Generate financial reports in Leo Counter with KPI cards, income vs. expense charts, category distribution, and budget usage. Filter by period, category, and account."
---
...
The `ReporteController` renders the Inertia page and seeds the initial data. The `useReporteApi` hook then takes over client-side, calling the report API endpoint which dispatches `GenerateFinancialReportQuery` through `GenerateReportHandler`. That handler runs multiple assemblers in parallel:
